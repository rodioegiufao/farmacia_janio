...
function normalizarImagemBase64(valor) { const texto = String(valor || ""); const match = texto.match(/^data:image\/(png|jpeg|jpg);base64,(.+)$/); if (!match) return null; return { buffer: Buffer.from(match[2], "base64"), extension: match[1] === "jpg" ? "jpeg" : match[1] }; }
...
function gerarBlocoImagens(zip, imagens = []) { const lista = Array.isArray(imagens) ? imagens : []; let rels = zip.file("word/_rels/document.xml.rels").asText(); let relInsert = ""; let xml = ""; let contador = 0; lista.forEach((imagem, index) => { const normalizada = normalizarImagemBase64(imagem?.arquivo); if (!normalizada) return; contador += 1; const rId = `rIdMemoImagem${Date.now()}${contador}`; const extension = normalizada.extension; zip.file(`word/media/memorando-imagem-${contador}.${extension}`, normalizada.buffer); relInsert += `<Relationship Id="${rId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/memorando-imagem-${contador}.${extension}"/>`; garantirContentTypeImagem(zip, extension); const titulo = textoLimpo(imagem.titulo) || `Figura ${index + 1} — Imagem do projeto`; const descricao = textoLimpo(imagem.descricao); xml += paragrafoXml(titulo, { alignment: "center", firstLine: 0, before: contador === 1 ? 120 : 260, after: 80, bold: true }); xml += imagemXml(rId, contador, normalizada.buffer, extension); if (descricao) xml += paragrafoXml(descricao, { alignment: "both", firstLine: 0, before: 40, after: 180, italic: true }); }); if (relInsert) zip.file("word/_rels/document.xml.rels", rels.replace("</Relationships>", `${relInsert}</Relationships>`)); return xml || paragrafoXml("Não foram inseridas imagens do projeto."); }
