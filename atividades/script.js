...
function criarModalPlannerAutomatico(titulo, conteudo, botoes) {
  ...
  modal.innerHTML = `<div class="planner-modal-card${configuracaoInicial ? " planner-auto-setup-card" : ""}" role="dialog" aria-modal="true" aria-labelledby="plannerAutoTitle"><header class="planner-auto-setup-header"><div><h2 id="plannerAutoTitle">${escapeHtml(titulo)}</h2></div><button type="button" class="planner-modal-close" data-auto-close aria-label="Fechar">&times;</button></header><div class="planner-auto-content${configuracaoInicial ? " planner-auto-setup-body" : ""}">${conteudo}</div><footer class="form-actions planner-auto-setup-footer">${botoes.map((b, i) => `<button type="button" class="${i ? "secondary" : "primary"}" data-auto-action="${escapeHtml(b.valor)}">${escapeHtml(b.texto)}</button>`).join("")}</footer></div>`;
  if (configuracaoInicial) modal.classList.add("planner-auto-setup-modal");
  modal.innerHTML = `<div class="planner-modal-card${configuracaoInicial ? " planner-auto-setup-card" : ""}" role="dialog" aria-modal="true" aria-labelledby="plannerAutoTitle"><header class="planner-auto-setup-header"><div><h2 id="plannerAutoTitle">${escapeHtml(titulo)}</h2></div><button type="button" class="planner-modal-close" data-auto-close aria-label="Fechar">&times;</button></header><div class="planner-auto-content${configuracaoInicial ? " planner-auto-setup-body" : ""}">${conteudo}</div><footer class="form-actions planner-auto-setup-footer">${botoes.map((b, i) => `<button type="button" class="${i ? "secondary" : "primary"}" data-auto-action="${escapeHtml(b.valor)}">${escapeHtml(b.texto)}</button>`).join("")}</footer></div>`;
