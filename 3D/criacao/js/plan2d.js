...
Object.assign(this, { c: canvas, ctx: canvas.getContext("2d"), store, onStatus, onError, scale: 0.7, off: { x: 0, y: 0 }, points: [], preview: null, primitiveStart: 0, awaitingLineEndpoint: false, lastTool: null, drag: null, editVertexDrag: null, moveDrag: null, typedDistance: "" });
  ...
  down(e) {
    ...
    if (this.points.length > 2 && Math.hypot(point.x - this.points[0].x, point.y - this.points[0].y) < 15 / this.scale) {
      this.preview = null;
      this.awaitingLineEndpoint = tool === "line";
      if (this.s.creationSession?.active) this.store.setTemporaryPoints(this.currentDraft());
      return this.draw();
    }
  ...
  finish() { try { const cs = this.s.creationSession, step = cs?.step; const tool = this.activeTool(); let pts = this.composePrimitive(this.points); const closed = !cs || !["path","axis"].includes(step); this.validate(pts, closed); if (this.s.editMode === "profileEdit") { this.store.finishProfileEdit(pts); this.points=[]; this.preview=null; this.primitiveStart=0; this.awaitingLineEndpoint=false; this.draw(); return; } if (cs?.active) { const color = cs.operation === "void" ? "#f36b2d" : "#ff00cc"; if (step === "path" || step === "axis") { const path=this.store.addPath(pts.map((p)=>({...p,z:0})),{name: step === "axis" ? "Eixo de revolução" : "Caminho"}); this.store.advanceCreationStep(step === "axis" ? {axisId:path.id,pathId:path.id}:{pathId:path.id}); } else { this.store.addProfile(pts,{name:"Perfil de criação",material:{color}}); try { this.store.advanceCreationStep({profileId:this.store.state.selectedElementId}); } catch (err) { this.onError(err.message); } } } else if (tool === "line") this.store.addPath(pts.map((p)=>({...p,z:0}))); else this.store.addProfile(pts); this.points=[]; this.preview=null; this.primitiveStart=0; this.awaitingLineEndpoint=false; this.draw(); } catch (err) { this.onError(err.message); } }
  cancel() { this.points = []; this.preview = null; this.typedDistance = ""; this.primitiveStart = 0; this.awaitingLineEndpoint = false; this.editVertexDrag = null; if (this.s?.editMode === "profileEdit") this.store.cancelProfileEdit(); if (this.s?.creationSession?.active) this.store.cancelCreationSession(); this.draw(); }
