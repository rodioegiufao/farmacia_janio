...
// A implementação Excel fica fora do grafo inicial e só é baixada ao primeiro uso.
let excelFeaturePromise;
function getExcelFeature() {
    excelFeaturePromise ||= import("./features/excel.js");
...
async function loadAssociationDefinitionsFromExcel(...args) {
    return (await getExcelFeature()).loadAssociationDefinitionsFromExcel(...args);
}
async function loadAssociaUnitsFromExcel(...args) {
...
createGroundGrid();
// Excel é carregado somente quando uma ferramenta de materiais/orçamento o solicita.
...
function handleProjectSelectChange(event) {
    ...
    if (!projectKey || !hasProjectConfig(projectKey)) {
        return;
    }

    const { models, transforms } = getProjectConfig(projectKey, "authenticated");
...
function handleConfiguredProjectSelection(projectKey) {
    const { models, transforms } = getProjectConfig(projectKey, "authenticated");
    handleModelSelection(models, transforms, projectKey);
}
...
if (projectFromDataset && hasProjectConfig(projectFromDataset)) {
    const { models, transforms } = getProjectConfig(projectFromDataset, "authenticated");
