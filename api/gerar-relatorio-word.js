...
function imagemGanttXml(rId,indice,largura,altura) { const cx=5852160,proporcao=Number(largura)>0&&Number(altura)>0?Number(altura)/Number(largura):0.55,cy=Math.round(cx*proporcao); return `<w:p><w:pPr><w:keepNext/><w:jc w:val="center"/><w:spacing w:before="100" w:after="180"/></w:pPr><w:r><w:drawing><wp:inline distT="0" distB="0" distL="0" distR="0" xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing"><wp:extent cx="${cx}" cy="${cy}"/><wp:docPr id="${1100+indice}" name="Gantt da execução real ${indice}"/><a:graphic xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"><a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:nvPicPr><pic:cNvPr id="${1100+indice}" name="relatorio-gantt-${indice}.png"/><pic:cNvPicPr/></pic:nvPicPr><pic:blipFill><a:blip r:embed="${rId}" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"/><a:stretch><a:fillRect/></a:stretch></pic:blipFill><pic:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="${cx}" cy="${cy}"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></pic:spPr></pic:pic></a:graphicData></a:graphic></wp:inline></w:drawing></w:r></w:p>`; }
function prepararGanttAnexo(zip,gantt={}) { const explicacao=paragrafoCorpoXml("O Gantt apresenta a distribuição temporal das atividades efetivamente registradas no período analisado. Os blocos representam os dias em que houve execução registrada e não constituem programação futura ou linha de base contratual."); const imagens=Array.isArray(gantt.imagens)?gantt.imagens:[];let rels="",xml=explicacao,validas=0; imagens.forEach((item)=>{const buffer=normalizarImagemBase64(item?.imagem);if(!buffer)return;validas+=1;const rId=`rIdRelGantt${validas}`,nome=imagens.length===1?"relatorio-gantt.png":`relatorio-gantt-${validas}.png`;zip.file(`word/media/${nome}`,buffer);rels+=`<Relationship Id="${rId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/${nome}"/>`;if(item.obra)xml+=criarTituloGrafico(item.obra);xml+=imagemGanttXml(rId,validas,item.largura,item.altura);}); if(!validas)xml+=paragrafoXml("Não foram identificadas atividades com intervalo válido para representação no Gantt no período analisado.");adicionarRelationships(zip,rels);garantirContentTypePng(zip);return xml; }
