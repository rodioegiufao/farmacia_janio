...
export class Store {
  ...
  pushHistory() {
    ...
    if (this.state.history.undo.length > 60) this.state.history.undo.shift();
  ...
  addWall(w) {
    this.pushHistory();
    this.state.walls.push(w);
    this.state.selectedWallId = w.id;
    this.emit();
  }
  updateWall(id, p) {
    this.pushHistory();
    this.state.walls = this.state.walls.map((w) =>
      w.id === id ? { ...w, ...p } : w,
    );
    this.emit();
  }
  deleteSelected() {
    const id = this.state.selectedWallId;
    if (!id) return false;
    this.pushHistory();
  ...
  loadSaved() {
    ...
    if (!raw) return false;
    this.replace(validateProject(JSON.parse(raw)));
    return true;
  }
  replace(project) {
    this.state = {
...
export function makeWallName(n) {
  return `Parede ${String(n).padStart(3, "0")}`;
}
export function uuid() {
  return crypto.randomUUID
    ? crypto.randomUUID()
    : `wall-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}
...
export function validateProject(data) {
  ...
  return {
    ...
    walls: data.walls.map((w, i) => {
      if (!w.start || !w.end || wallLength(w) <= 0)
        throw new Error(`Parede inválida na posição ${i + 1}.`);
      return {
        ...w,
        id: w.id || uuid(),
        name: w.name || makeWallName(i + 1),
        height: Number(w.height) || 2.8,
        thickness: Number(w.thickness) || 0.15,
        baseElevation: Number(w.baseElevation) || 0,
        levelId: "ground-floor",
        material: w.material || { name: "Alvenaria", color: "#d2b48c" },
      };
    }),
  };
}