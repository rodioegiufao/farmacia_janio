...
function cabecalho(dias, l) {
  ...
  const segunda = linha([celula("PROCESSO", l.processo, { fill: CORES.cabecalho, cor: "FFFFFF", bold: true }), celula("HORAS", l.horas, { fill: CORES.cabecalho, cor: "FFFFFF", bold: true }), ...dias.map((x) => { const dt = data(x); return celula(`${x.slice(8)}\n${SEMANAS[dt.getUTCDay()]}`, l.dia, { fill: CORES.cabecalho, cor: "FFFFFF", bold: true, tamanho: 14, noWrap: true }); })], true);
...
function gerarGanttTabelaXml(gantt = {}) {
  const obras = (gantt.obras || []).filter((o) => (o.dias || []).length); if (!obras.length) return { xml: paragrafo("Não foram identificadas atividades com intervalo válido para representação no Gantt no período analisado."), paisagem: false };
  const blocos = periodosTemporais(gantt.periodo), paisagem = listarDias(gantt.periodo.inicio, gantt.periodo.fim).length > 14;
  ...
  xml += paragrafo(`Período: ${formatarData(gantt.periodo.inicio)} a ${formatarData(gantt.periodo.fim)}`, { bold: true, after: 40 }) + paragrafo(`Horas registradas: ${formatarHoras(gantt.totalHoras)}`, { bold: true, after: 140 }) + paragrafo("SÍNTESE TEMPORAL", { bold: true, keepNext: true }) + sintese(obras);
  let indice = 0; blocos.forEach((bloco, bi) => { if (blocos.length > 1) xml += paragrafo(`${MESES[data(bloco.inicio).getUTCMonth()]}/${data(bloco.inicio).getUTCFullYear()}`, { bold: true, keepNext: true, before: 220 }); obras.forEach((obra) => { const diasAtivos = (obra.dias || []).filter((x) => x >= bloco.inicio && x <= bloco.fim); if (!diasAtivos.length) return; indice += 1; const codigo = obra.codigo ? ` — ${obra.codigo}` : ""; xml += paragrafo(`B.${indice} —${codigo} — ${obra.nome}`, { bold: true, keepNext: true, before: 220, after: 80 }) + tabelaObra(obra, bloco.dias, paisagem); }); });
  if (paisagem) xml = `<w:p><w:pPr><w:sectPr><w:type w:val="nextPage"/><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:top="1134" w:right="1134" w:bottom="1134" w:left="1134"/></w:sectPr></w:pPr></w:p>` + xml;
  return { xml, paisagem };
}
...
module.exports = { gerarGanttTabelaXml, aplicarPaisagemUltimaSecao, periodosTemporais, listarDias, formatarHoras, CORES };