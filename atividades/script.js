...
function intervaloVisivelPlannerGantt(global) {
  ...
  if (plannerGanttEscala === "semana") { inicio.setDate(inicio.getDate() - inicio.getDay()); fim.setTime(inicio.getTime()); fim.setDate(fim.getDate() + 6); }
...
function divisoriasSemanasPlannerGantt(dias) { return dias.map((iso, indice) => new Date(`${iso}T12:00:00`).getDay() === 0 && indice ? `<span class="planner-gantt-week-divider" style="grid-column:${indice + 1}"></span>` : "").join(""); }
...
function abrirAtividadesVinculadasGantt(no, gatilho) {
  ...
  const minutos = lista.reduce((total, atividade) => total + calcularMinutosAtividadePlanner(atividade), 0);
  ...
  document.getElementById("plannerGanttActivitiesList").innerHTML = lista.length ? `<div class="planner-linked-table">${lista.map((a) => `<div class="planner-linked-activity-row"><time>${escapeHtml(formatarData(a.data_inicio || a.data_termino))}</time><strong>${escapeHtml(a.colaborador || a.colaborador_nome || "-")}</strong><span>${escapeHtml(a.trabalhos || a.descricao || "-")}</span><span>${formatarMinutosPlanner(calcularMinutosAtividadePlanner(a))}</span><span>${escapeHtml(a.status || "-")}</span></div>`).join("")}</div>` : '<p class="empty planner-linked-empty">Nenhuma atividade vinculada foi identificada para este elemento no período e filtros selecionados.</p>';
...
function renderizarPlannerGantt(checklists) {
  ...
  const titulo=plannerGanttEscala==="semana"?`SEMANA ${PLANNER_GANTT.obterNumeroSemanaDomingo(intervalo.inicio)}`:plannerGanttEscala==="mes"?new Date(`${intervalo.inicio}T12:00:00`).toLocaleDateString("pt-BR",{month:"long",year:"numeric"}).toUpperCase():`${rotuloDataGantt(intervalo.inicio)} — ${rotuloDataGantt(intervalo.fim)}`;
...
function calcularMinutosAtividadePlanner(a) { const rateados = Number(a?.minutosRateados); return Number.isFinite(rateados) && rateados >= 0 ? rateados : calcularMinutosAtividadeBanco(a); }
