...
const registro = (projeto, fase, item, horas) => ({ projeto, fase, item, horas });
...
const projetos = api.agruparHorasPorFaseDashboard([registro("BT", "Distribuição", "", 10), registro("CFTV", "Distribuição", "", 5)], calcular);
assert.equal(projetos.categorias.length, 2);
...
assert.equal(itens.categorias.filter((item) => item.item === "Iluminação").length, 2);
