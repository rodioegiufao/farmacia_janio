...
const guid = () =>
  Math.random().toString(36).slice(2, 24).padEnd(22, "0").slice(0, 22);
...
function createSpatialStructure(writer, projectName) {
  ...
  const localPlacement = writer.add("IFCLOCALPLACEMENT", [
    "$",
...
function addExtrusion(writer, state, extrusion, profile, params, context) {
  ...
  const points3d = profile.points.map((point) =>
    pointTo3D(point, profile.view, offset),
  );
  ...
  const elementOrigin = writer.add("IFCCARTESIANPOINT", [
    `(${num(min.x)},${num(min.y)},${num(min.z)})`,
  ]);
  const axisZ = writer.add("IFCDIRECTION", ["(0.,0.,1.)"]);
  const axisX = writer.add("IFCDIRECTION", ["(1.,0.,0.)"]);
  const placement3d = writer.add("IFCAXIS2PLACEMENT3D", [
    writer.ref(elementOrigin),
    writer.ref(axisZ),
    writer.ref(axisX),
  ]);
  const localPlacement = writer.add("IFCLOCALPLACEMENT", [
    writer.ref(context.localPlacement),
    writer.ref(placement3d),
  ]);
  const pointIds = local2d.map((point) =>
    writer.add("IFCCARTESIANPOINT", [`(${num(point.x)},${num(point.y)})`]),
  );
  if (pointIds[0] !== pointIds.at(-1)) pointIds.push(pointIds[0]);
  const polyline = writer.add("IFCPOLYLINE", [
    `(${pointIds.map((id) => writer.ref(id)).join(",")})`,
  ]);
  ...
  const solidPlacement = writer.add("IFCAXIS2PLACEMENT3D", [
    writer.ref(elementOrigin),
    writer.ref(axisZ),
    writer.ref(axisX),
  ]);
  const dir = writer.add("IFCDIRECTION", ["(0.,0.,1.)"]);
  const solid = writer.add("IFCEXTRUDEDAREASOLID", [
    writer.ref(profileDef),
    writer.ref(solidPlacement),
    writer.ref(dir),
    num(depth),
  ]);
  ...
  const productShape = writer.add("IFCPRODUCTDEFINITIONSHAPE", [
    "$",
    "$",
    `(${writer.ref(representation)})`,
  ]);
  const type = getElementType(state.category, writer.schema);
  const element = writer.add(type, [
    `'${guid()}'`,
    "$",
    `'${sanitize(extrusion.name)}'`,
    "$",
    "$",
    writer.ref(localPlacement),
...
export function exportFamilyToIfc(state, schema = IFC_SCHEMA.IFC4) {
  ...
  const elementIds = [];
  for (const extrusion of state.extrusions.filter(
    (item) => item.visible !== false,
  )) {
    const profile = state.profiles.find(
      (item) => item.id === extrusion.profileId && item.visible !== false,
    );
    if (!profile || profile.points.length < 3) continue;
    elementIds.push(
      addExtrusion(writer, state, extrusion, profile, params, context),
    );
  }
  if (!elementIds.length)
    throw new Error(
      "Crie ao menos uma extrusão visível antes de exportar IFC.",
    );
  writer.add("IFCRELCONTAINEDINSPATIALSTRUCTURE", [
    ...
    "$",
    "$",
    `(${elementIds.map((id) => writer.ref(id)).join(",")})`,
